...
func (g *Generator) TypeToInterfaceType(goType string) string {
	...
	// This handles cases where the type mapping didn't catch it
	...
	if strings.Contains(goType, ".") {
		...
		if len(parts) == 2 {
			...
			// For other frameworks, use objectivec.IObject (the interface from github.com/tmc/appledocs/generated/objectivec)
			currentLevel := getFrameworkLevel(strings.ToLower(g.Framework))
			targetLevel := getFrameworkLevel(pkg)
	...
	if !g.IsClassType(goType) && !g.IsClassType(baseType) {
		...
		if found {
			...
			if currentLevel >= 0 && targetLevel > currentLevel {
				// Hierarchy violation - use objectivec.IObject
				Debug.TypeMap("cross-framework type violates hierarchy", goType, "objectivec.IObject",
					"goType", goType,
				...
				if debugTypeAnnotations {
					return fmt.Sprintf("objectivec.IObject /* type-resolution: cross-framework=%s.%s, objc-type=%s, reason=hierarchy-violation */", frameworkPkg, goType, goType)
				}
				return "objectivec.IObject /* cross-framework: " + goType + " */"
			}
			...
			if strings.Contains(goType, ".") {
				...
				parts := strings.SplitN(goType, ".", 2)
				// Check if same framework first
				if strings.ToLower(g.Framework) == parts[0] {
				...
				// Check if same framework first
			if strings.ToLower(g.Framework) == frameworkPkg {
				qualifiedType = goType
			} else {
		...
		if baseType != goType {
			...
			if baseFound {
				...
				if currentLevel >= 0 && targetLevel > currentLevel {
					// Hierarchy violation - use objectivec.IObject
					Debug.TypeMap("cross-framework type violates hierarchy (stripped)", baseType, "objectivec.IObject",
						"baseType", baseType,
						"currentFramework", g.Framework,
						"currentLevel", currentLevel,
						...
						"returning", "objectivec.IObject")
					if debugTypeAnnotations {
						return fmt.Sprintf("objectivec.IObject /* type-resolution: cross-framework=%s.%s, objc-type=%s, stripped-from=%s, reason=hierarchy-violation */", baseFrameworkPkg, baseType, baseType, goType)
					}
					return "objectivec.IObject /* cross-framework: " + baseType + " */"
				}
				...
				if strings.Contains(baseType, ".") {
					...
					// Not qualified yet, add framework package
					// Check if same framework first
				if strings.ToLower(g.Framework) == baseFrameworkPkg {
